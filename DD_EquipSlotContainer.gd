...
@onready var inventoryControllerRef:DD_InventoryController = DragDropController.GetControllerRef()
...
func EquipItem():
	...
	match equippedItem.itemType:
		Globals.ItemTypes.ARMOUR:
			...
			_tooltipText +=  "\nThinginess: " + str(equippedItem.exampleFloat)
		Globals.ItemTypes.WEAPON:
			_tooltipText += "\nDisappointment: " + str(equippedItem.exampleInt)
